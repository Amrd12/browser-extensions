console.log("Akwam Donwload     ::    ADDED");
   ...
   function series() {
    ...
    const reversedLinks = Array.from(links)
    const akwamlinks = []
    reversedLinks.forEach(
      (div)=>{
        akwamlinks.push(div.getElementsByTagName("a")[0].getAttribute('href'))
      }
    )
    chrome.runtime.sendMessage({ action: "SendArray", array :akwamlinks} , function(response){
      console.log(response.copy)
      copy1(response.copy) ; console.log("Akwam Donwload  ::  copied") 
    })
  }
...
if (targetDiv) {
  ...
  trdocument.getElementsByClassName("row py-4")[0].getElementsByTagName("div")[0].appendChild(inputElement);
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse)=>{
      if(message.action==="tab1"){
     const link = document.querySelector('a.link-btn.link-download.d-flex.align-items-center.px-3').getAttribute('href').replace('http://', 'https://');
      console.log(link)
     sendResponse({link1 : link})
      }
    })
    chrome.runtime.onMessage.addListener((message, sender, sendResponse)=>{
      if(message.action==="tab2"){
     const link = document.querySelector('div.content').querySelector('a.download-link').getAttribute('href');
     console.log(link)

     sendResponse({link2 : link})
      }
    })  
    chrome.runtime.onMessage.addListener((message, sender, sendResponse)=>{
      if(message.action==="tab3"){

     const link = document.querySelector('a.link.btn.btn-light').getAttribute('href');
     console.log(link)

     sendResponse({link3 : link})
      }
    })